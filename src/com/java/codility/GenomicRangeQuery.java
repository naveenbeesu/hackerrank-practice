...
public class GenomicRangeQuery {
	...
	public static int[] solution(String S, int[] P, int[] Q) {
		...
		for(int i=0;i<P.length;i++) {
			nucleotide = S.substring(P[i], Q[i]+1);
			if(nucleotide.contains("A")) {
				result[i] = 1;
				continue;
			} if(nucleotide.contains("C")) {
				result[i] = 2;
				continue;
			} if(nucleotide.contains("G")) {
				result[i] = 3;
				continue;
...
/*
 * GenomicRangeQuery START Find the minimal nucleotide from a range of sequence
 * DNA. A DNA sequence can be represented as a string consisting of the letters
